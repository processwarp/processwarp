...
// __builtin_va_arg関数。
BuiltinPost BuiltinVaArg::arg(Process& proc, Thread& thread,
  ...
  thread.memory->set<vaddr_t>(arglist, tmp_addr);

  return BP_NORMAL;
}

// llvm.va_copy関数。
BuiltinPost BuiltinVaArg::copy(Process& proc, Thread& thread,
                               BuiltinFuncParam p, vaddr_t dst,
                               std::vector<uint8_t>& src) {
  int seek = 0;
  // i8* destarglistを取り出す。
  ...
  vaddr_t src_addr = thread.memory->get<vaddr_t>(srcarglist);
  thread.memory->set<vaddr_t>(dstarglist, src_addr);

  return BP_NORMAL;
}

// llvm.va_end関数。
BuiltinPost BuiltinVaArg::end(Process& proc, Thread& thread,
                              BuiltinFuncParam p, vaddr_t dst,
                              std::vector<uint8_t>& src) {
...
// llvm.va_start関数。
BuiltinPost BuiltinVaArg::start(Process& proc, Thread& thread,
                                BuiltinFuncParam p, vaddr_t dst,
                                std::vector<uint8_t>& src) {
  int seek = 0;
  // i8* arglistを取り出す
