...
// byte-swap関数。
BuiltinPost BuiltinBit::bswap(Process& proc, Thread& thread, BuiltinFuncParam p,
  ...
  case 16: {
    ...
    thread.memory->set<uint16_t>(dst, work);
  } break;
  ...
  case 32: {
    ...
    thread.memory->set<uint32_t>(dst, work);
  } break;
  ...
  case 64: {
    ...
    thread.memory->set<uint64_t>(dst, work);
  } break;
