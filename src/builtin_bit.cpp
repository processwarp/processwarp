...
BuiltinPost BuiltinBit::bswap(VMachine& vm, Thread& th, BuiltinFuncParam p,
  ...
  switch(p.i64) {
  case 16: {
    uint16_t work = VMachine::read_builtin_param_i16(src, &seek);
    work = (0xff00 & (work << 8)) || (0x00ff & (work >> 8));
    *reinterpret_cast<uint16_t*>(vm.get_raw_addr(dst)) = work;
  } break;

  case 32: {
    uint32_t work = VMachine::read_builtin_param_i32(src, &seek);
  ...
  case 64: {
    uint64_t work = VMachine::read_builtin_param_i64(src, &seek);
