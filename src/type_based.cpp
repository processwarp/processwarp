...
template <typename T> void TypeExtended<T>::type_cast(uint8_t* dst, vaddr_t type, uint8_t* src) {
  ...
  case BasicType::TY_UI8: {
    *reinterpret_cast<uint8_t*>(dst) = static_cast<uint8_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_UI16: {
    *reinterpret_cast<uint16_t*>(dst) = static_cast<uint16_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_UI32: {
    *reinterpret_cast<uint32_t*>(dst) = static_cast<uint32_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_UI64: {
    *reinterpret_cast<uint64_t*>(dst) = static_cast<uint64_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_SI8: {
    *reinterpret_cast<int8_t*>(dst) = static_cast<int8_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_SI16: {
    *reinterpret_cast<int16_t*>(dst) = static_cast<int16_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_SI32: {
    *reinterpret_cast<int32_t*>(dst) = static_cast<int32_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_SI64: {
    *reinterpret_cast<int64_t*>(dst) = static_cast<int64_t>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_F32: {
    *reinterpret_cast<float*>(dst) = static_cast<float>(*reinterpret_cast<T*>(src));
  } break;

  case BasicType::TY_F64: {
    *reinterpret_cast<double*>(dst) = static_cast<double>(*reinterpret_cast<T*>(src));
  } break;
